...
public class LoginElementsPresentTest {
  ...
  @Test
  public void loginElementPresent() {
    ...
    {
      List<WebElement> elements = driver.findElements(By.cssSelector(".ant-btn"));
      assert(elements.size() > 0);
    }
    {
      List<WebElement> elements = driver.findElements(By.cssSelector(".login-form-forgot"));
      assert(elements.size() > 0);
    }
    driver.close();
  }
